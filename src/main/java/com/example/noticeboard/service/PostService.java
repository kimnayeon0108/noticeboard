...
public class PostService {
    ...
    public ResPostDto addPost(ReqCreatePostDto reqCreatePostDto, MultipartFile[] multipartFiles) throws IOException {
    ...
    public ResPostDto updatePost(long postId, ReqUpdatePostDto reqUpdatePostDto, MultipartFile[] multipartFiles) throws IOException {
        Post post = postRepository.findById(postId)
                                  .orElseThrow(() -> new BaseException(ErrorCode.POST_NOT_FOUND));
        User user = userRepository.findById(reqUpdatePostDto.getUserId())
                                  .orElseThrow(() -> new BaseException(ErrorCode.USER_NOT_FOUND));
        ...
        Category category = categoryRepository.findById(reqUpdatePostDto.getCategoryId())
                                              .orElseThrow(() -> new BaseException(ErrorCode.CATEGORY_NOT_FOUND));
        post.update(reqUpdatePostDto, category);

        // 기존 게시글 파일 삭제
        postFileRepository.deleteAllByPostIdIn(List.of(postId));          //Todo: 바뀐 것만 바뀌도록 , 전부 다 삭제 X
